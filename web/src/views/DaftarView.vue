<template>
  <div class="onboarding-page">
    <div class="onboarding-card">
      ...
      <!-- Sukses submit -->
      <div v-if="result" class="state-box success">
      ...
      <form v-else @submit.prevent="submit">
        ...
        <p class="sub">Isi data lengkap Anda di bawah ini. Setelah submit, username &amp; password akan dikirim ke email yang Anda daftarkan. Akun akan direview admin sebelum akses modul dibuka.</p>
        ...
        <h3 class="section-title">Verifikasi</h3>
        <div class="form-grid">
          <div class="field field-wide">
            <label>{{ captchaQuestion || 'Memuat pertanyaan...' }} <span class="req">*</span></label>
            <div class="captcha-row">
              <input v-model="captchaAnswer" type="text" inputmode="numeric" required />
              <button type="button" class="btn-refresh" :disabled="loadingCaptcha" @click="fetchCaptcha">↻</button>
            </div>
          </div>
        </div>
...
async function fetchDepartemenList() {
  try {
    const r = await api.get('/onboarding-open/departemen')
...
async function fetchCaptcha() {
  ...
  captchaAnswer.value = ''
  try {
    const r = await api.get('/onboarding-open/captcha')
    captchaQuestion.value = r.data.data.question
    captchaToken.value = r.data.data.captcha_token
  } catch {
    captchaQuestion.value = 'Gagal memuat captcha, coba muat ulang halaman'
  } finally {
...
async function submit() {
  ...
  try {
    ...
    fd.append('captcha_token', captchaToken.value)
    fd.append('captcha_answer', captchaAnswer.value)
    if (foto.value) fd.append('foto', foto.value)

    const r = await api.post('/onboarding-open', fd, {
...
onMounted(async () => {
  await Promise.all([fetchDepartemenList(), fetchCaptcha()])
})
</script>
